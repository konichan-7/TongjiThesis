...
在跳跃落地或快速伸腿工况下，链条会承受较大的瞬时拉力。以腿部电机峰值扭矩 $T_{\text{peak}} = 12.5\,\mathrm{N\cdot m}$、链轮等效直径 $d = 0.04907\,\mathrm{m}$ 估算，有效圆周力为

\begin{equation}
  F_t = \frac{2 T_{\text{peak}}}{d}
  = \frac{2 \times 12.5}{0.04907}
  \approx 509.5\,\mathrm{N}.
\end{equation}
...
上述估算按较保守的工况取值。实际样机为双腿协同支撑，且每条腿由多个连杆和电机共同承担负载，单电机峰值需求会低于该估算。综合质量、体积和控制接口，腿部关节驱动器选用额定扭矩 $3.5\,\mathrm{N\cdot m}$、峰值扭矩 $12.5\,\mathrm{N\cdot m}$ 的无刷减速电机。该电机支持力矩、速度和位置控制模式，满足站立平衡和腿部快速调节需求。

\begin{figure}[H]
  \centering
  \includegraphics[width=0.9\textwidth]{figures/ch02/image7.png}
  \caption{腿部关节驱动电机}
  \label{fig:leg-motor}
\end{figure}
...
双腿支撑时，单腿持续负载约减半，即约为 $2.6\,\mathrm{N\cdot m}$。所选电机额定扭矩为 $3.5\,\mathrm{N\cdot m}$，可覆盖该需求，并为姿态调节和小扰动恢复留出余量。

\subsection{轮毂电机}

轮毂电机按第 2.5 节的速度、加速和爬坡需求选型。本文选用额定扭矩为 $2.5\,\mathrm{N\cdot m}$ 的减速轮毂电机，并将其直接布置在足端轮组内部。该方案省去额外传动链，轮系角度和速度也可直接用于估计前进位移与速度。

\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\textwidth]{figures/ch02/image8.png}
  \caption{足端轮毂电机}
  \label{fig:wheel-motor}
\end{figure}

根据前述计算，双轮驱动下单轮连续扭矩需求约为 $0.7 \sim 1.0\,\mathrm{N\cdot m}$，所选轮毂电机额定扭矩为 $2.5\,\mathrm{N\cdot m}$，可满足平地加速和一定坡度爬升需求。直接驱动形式还能减少传动回差，使轮系状态估计更简单。

\subsection{传感器与控制硬件配置}

第四章的控制系统需要实时获得机体姿态、轮组运动和腿部关节状态。样机硬件按状态感知、实时计算和驱动执行三部分配置。

状态感知部分包括惯性测量单元、轮组编码器和腿部关节编码器。惯性测量单元测量机体俯仰角、滚转角及对应角速度，是平衡控制和滚转补偿的主要反馈来源。轮组编码器测量左右轮转角和转速，结合轮半径可得到前进位移、前进速度和差速转向相关状态。腿部关节编码器测量主动关节角度，再结合第三章的闭环机构运动学关系解算虚拟腿长、虚拟腿摆角及其变化率。

实时计算部分由主控板完成，主要包括传感器数据采集、状态估计、LQR 与 VMC 控制律计算、滚转补偿以及执行层约束处理。本文控制算法以矩阵运算、解析映射和小规模约束投影为主，算力需求相对可控，适合在嵌入式控制器上运行。主控板与电机驱动器之间通过 CAN 或串行总线通信，以保证多电机指令同步和反馈更新频率。

驱动执行部分由腿部关节电机和轮毂电机完成。腿部关节电机接收力矩或等效电流指令，用于产生虚拟腿支撑力和腿摆转矩；轮毂电机接收速度或力矩指令，用于前进、制动和转向。电池、电源转换模块和保护电路布置在躯干内部，为主控板、传感器和驱动器供电。

\subsection{整机关键参数汇总}

样机关键参数汇总如\tabref{tab:robot-key-parameters}所示。这些参数后续将用于建模和控制器参数选取。

\begin{table}[H]
  \centering
  \caption{整机关键参数汇总}\label{tab:robot-key-parameters}
  \renewcommand{\arraystretch}{1.25}
  \begin{tabular}{p{4cm}p{4cm}p{5cm}}
    \toprule
    参数 & 数值或配置 & 说明 \\
    \midrule
    整机目标质量 & $\leq 5\,\mathrm{kg}$ & 用于执行器校核与控制参数设计 \\
    自由度配置 & $6\,\mathrm{DOF}$ & 左右腿部驱动与双轮驱动构成基本运动自由度 \\
    最大移动速度 & $\geq 2\,\mathrm{m/s}$ & 轮式移动性能指标 \\
    可跨越台阶高度 & $\geq 10\,\mathrm{cm}$ & 腿部伸缩和越障能力指标 \\
    轮子半径 & $0.06\,\mathrm{m}$ & 用于轮速与前进速度换算 \\
    腿部有效行程 & 约 $256\,\mathrm{mm}$ & 由连杆最大伸展与最小收缩高度估算 \\
    关节电机额定/峰值扭矩 & $3.5/12.5\,\mathrm{N\cdot m}$ & 用于腿部支撑、伸缩和摆动控制 \\
    轮毂电机额定扭矩 & $2.5\,\mathrm{N\cdot m}$ & 用于平地移动和差速转向 \\
    主要传感器 & IMU、轮组编码器、关节编码器 & 用于姿态、轮系和腿部状态估计 \\
    \bottomrule
  \end{tabular}
\end{table}

从参数匹配关系看，腿部有效行程覆盖越障和缓冲需求；关节电机额定扭矩可满足双腿支撑下的持续负载要求，峰值扭矩可用于短时动态动作；轮毂电机额定扭矩相对于加速和爬坡估算值也有余量。样机结构在质量、运动能力和控制实现难度之间取了一个相对保守的折中。
