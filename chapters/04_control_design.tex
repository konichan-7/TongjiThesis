...
本章承接第三章的建模结果，重点讨论控制器的组织与实现。考虑到样机的计算资源、调参难度以及后续实验的可重复性，本文采用分层控制结构：纵向平衡与速度跟踪由 LQR 完成，腿长方向的支撑与缓冲由 VMC 生成，侧向姿态则通过滚转稳定环进行补偿。
这样处理可以把不同方向、不同时间尺度上的控制任务分开，避免在样机端引入过重的在线优化计算，同时保留足够的参数调节空间。后续 MuJoCo 仿真和第五章物理样机实验均基于该控制框架展开。

\section{总体架构}

双轮足机器人在运行时需要同时处理多个控制目标。沿前进方向，机器人应保持轮式倒立摆的平衡状态，并能够完成原地站立、速度跟踪和受扰恢复；在腿部方向，机构需要承担支撑、缓冲、机身高度调节以及抬腿、摆腿等动作；在侧向方向，机器人转向、经过坡面或受到横向扰动时，还需要抑制滚转角变化，避免单侧轮卸载或机体侧翻。与此同时，上述控制过程都必须在嵌入式控制器上实时运行，包括状态估计、控制量计算和执行器命令下发。
...
在该架构中，LQR、VMC 和滚转稳定环分别作用于不同的控制层级。LQR 根据线性化模型计算轮端力矩和腿摆方向控制量，用于维持俯仰平衡、速度响应和转向运动；VMC 将任务空间力矩转换到关节空间，同时根据腿长误差生成轴向支撑力，使腿部具有类似弹簧阻尼器的支撑和缓冲特性；滚转稳定环则在左右腿长参考值中加入差动修正，用于减小侧向姿态偏差。整体控制框图如\figref{fig:control-structure} 所示。
...
为了在保证多任务协调与稳定性的同时，适应单片机有限的计算资源，本文结合第三章的动力学模型推导，制定了以下三项核心控制设计原则：

\begin{enumerate}
  \item \textbf{基于物理运动模式的任务解耦}：双轮足机器人的运动呈现出多维耦合，如果直接采用集中式控制（如WBC），解算负担极大。本文根据机器人的运动特征，将整体目标拆解为两部分：矢状面的倒立摆动态平衡交由 LQR 控制，竖直方向的减震与支撑交由 VMC 独立调节。这种基于物理维度的解耦设计，不仅大幅降低了控制器的维度，还使得各通道的参数能够独立整定，提高了系统的可调试性。
  \item \textbf{基于虚功原理的力控映射机制}：面对闭环五连杆复杂的逆运动学问题，本策略并未直接求解各关节的期望位置，而是引入了虚拟模型控制（VMC）。通过在任务空间构造“虚拟弹簧-阻尼器”，不仅赋予了腿部良好的柔顺支撑特性，还利用虚功原理将虚拟受力精确映射为底层电机的关节力矩。这在理论上保证了控制量分配的准确性。
  \item \textbf{侧向姿态的代数补偿策略}：对于容易引发侧翻的滚转角，本策略没有引入复杂的零矩点（ZMP）方程进行在线规划，而是利用左右腿长差与机体侧倾的直接几何约束关系，设计了闭式的解析代数方程。通过直接对 VMC 的腿长目标施加差动修正，在不增加控制环路复杂度的前提下，以最直接的方式提高了机器人的侧向抗扰动能力。
\end{enumerate}

现有相关研究\cite{klemm2020lqr,yang2024integrated}为双轮足机器人的分层控制提供了重要参考。本文在此思路上，面向物理样机实际部署，进一步确立了这种精简的“定增益控制+几何补偿”技术路线。这种设计能够有效避免在线二次规划（QP）求解和复杂的参数自适应所带来的不确定性，保证了控制链路的清晰透明，最大程度地方便了后续仿真与样机实验的可重复性及安全性。
...
$\boldsymbol{Q}$ 和 $\boldsymbol{R}$ 矩阵的选取直接影响 LQR 的闭环性能。本文按照以下原则配置权重：(1) 俯仰角 $\theta_b$ 和角速度 $\dot{\theta}_b$ 的权重最高，以保证平衡恢复优先于响应系统输入；(2) 控制输入权重按驱动器的额定力矩和响应带宽设定，轮电机的控制代价低于腿部电机；(3) 前进位移 $s$ 和偏航角 $\phi$ 的权重相对较低，以避免在小速度误差时产生过大的瞬时驱动指令。
...
\end{equation}
其中，$m$ 为机器人的部分或整体等效质量，$\frac{1}{2} m g \cos\theta_{l,i}$ 为在此摆角下单腿分担的重力前馈项。\eqref{eq:leg-force-law} 使腿部在支撑阶段表现为具有等效刚度和阻尼的虚拟支撑单元，从而实现机体高度维持、落地缓冲和腿长振荡抑制。对应的虚拟控制关系如\figref{fig:vmc-response} 所示。与直接进行关节位置控制相比，这种写法更符合机器人与地面发生力学作用时的控制需求，也更便于在不同工况下通过少量参数完成行为切换。
...
MuJoCo 仿真模型如\figref{fig:mujoco-model} 所示，主要包括机体刚体、左右腿部连杆、足端轮组、转动关节、电机执行器以及地面接触参数。
...
由\figref{fig:balance-response} 可见，机体俯仰角在0.4s后收敛、左右腿角在0.3s后收敛，水平速度在0.2后收敛。整个控制过程没有明显震荡超调。
...
扰动恢复仿真用于评估控制器对外部冲击的鲁棒性。仿真中保持机器人处于原地平衡状态，分别对机体施加竖直方向和水平方向$2N$的短时脉冲扰动。竖直扰动主要考察 VMC 腿长支撑通道对瞬时载荷变化的缓冲能力，水平扰动主要考察 LQR 平衡控制器对机体俯仰偏差和前后运动趋势的恢复能力。扰动施加位置与方向如\figref{fig:impulse-scene} 所示，其中\figref{fig:vertical-impulse-scene} 和\figref{fig:horizon-impulse-scene} 分别对应竖直与水平脉冲扰动。
...
扰动响应结果如\figref{fig:impulse-disturbance-out} 所示。由\figref{fig:vertical-impulse-out} 可见，竖直扰动施加后，腿部轴向力首先出现短时峰值，随后在阻尼项作用下逐渐回到稳定支撑范围，左右腿长也未产生持续发散。该结果说明 VMC 腿长通道能够对瞬时竖直载荷进行缓冲。由\figref{fig:horizon-impulse-out} 可见，水平扰动施加后，机体俯仰角和前进速度出现瞬时偏差，轮端力矩随即产生反向调节，使机器人重新回到平衡状态。两类扰动结果表明，本文控制器在不同方向外部冲击下均具有一定恢复能力。

\subsection{腿长控制验证}

腿长控制验证用于检查 VMC 腿长通道对不同期望腿长的响应能力。仿真中保持机器人处于原地平衡状态，前进速度参考取 $\dot{s}_d=0$，分别对左右腿公共期望腿长 $l_d$ 施加阶跃激励和正弦激励。阶跃激励用于观察腿长指令突变时的快速调节能力，正弦激励用于观察连续高度变化下的跟踪能力。两类工况能够直接反映式\eqref{eq:leg-force-law} 所构造的虚拟弹簧阻尼控制律是否能够驱动腿部机构完成高度调节，并检验腿长变化过程中机体俯仰角和腿部轴向力是否保持在合理范围内。不同腿长激励下的响应如\figref{fig:leg-length-excitation} 所示。
...
首先，从控制目标和工程约束出发，说明了采用”LQR 运动 + VMC 腿长支撑 + 滚转稳定”这一分层架构的原因，并结合相关控制架构阐明了本文方案在任务分解、在线计算环节和工程实现路径上的取舍。随后，基于第三章的建模结果构建了运动控制器所需的线性状态空间模型，设计了 LQR 反馈律，并对增益矩阵的权重选取原则和参数敏感性进行了分析。在此基础上，通过 VMC 完成腿长支撑控制，并将 LQR 输出的腿摆控制量一并映射到关节空间，同时利用滚转辅助控制实现左右腿长参考修正。最后，给出了控制系统的状态估计与在线工作流程，并通过 MuJoCo 仿真对各类工况进行了验证。

从计算成本来看，本文分层架构的在线计算主要包括：(1) LQR 部分的 $10\times 10$ 矩阵乘法，复杂度为 O(100) 次浮点运算；(2) VMC 虚拟力的闭式代数计算，仅涉及平方根和少量乘加；(3) 滚转角补偿的三角函数查表；(4) 卡尔曼滤波器的矩阵运算。四个环节均无迭代收敛过程，符合样机嵌入式控制器对实时性和实现复杂度的要求。在样机的实际运行中，整个控制回路（含状态估计、控制量计算和力矩映射）的耗时始终控制在 1\,ms 以内，低于 5\,ms 的控制周期。上述内容共同构成了本文后续物理样机实验所依赖的控制系统实现基础。
