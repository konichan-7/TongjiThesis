...
为了在保证多任务协调与稳定性的同时降低控制器的在线计算负荷，结合第三章的模型推导及样机的实际工程约束，本文的控制架构主要遵循以下三项核心设计原则：

\begin{enumerate}
  \item \textbf{控制任务解耦化}：将前进方向的倒立摆平衡与速度跟踪、腿长方向的支撑缓冲、以及侧向姿态稳定，分别交由 LQR、VMC 和滚转几何补偿三个独立环节负责。各方向控制通道参数可独立调优，降低了系统耦合带来的调试难度。
  \item \textbf{核心算法轻量化（定增益 LQR）}：基于前文静态直立工作点处的线性化模型，离线求解代数 Riccati 方程获得恒定反馈增益矩阵。在线阶段仅需执行低阶矩阵乘法，确保在嵌入式系统 1\,kHz 的更新率下不造成明显的算力挤占。
  \item \textbf{侧向补偿几何化}：不采用 ZMP 轨迹调节或基于模型预测的侧向稳定策略，而是根据当前的滚转角与左右腿跨距等几何关系，以闭式解析的表达式直接对左右腿长参考值施加差动修正。
\end{enumerate}
