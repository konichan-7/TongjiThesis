...
本章承接第三章的建模结果，重点讨论控制器的组织与实现。考虑到样机的计算资源、调参难度以及后续实验的可重复性，本文采用分层控制结构：纵向平衡与速度跟踪由 LQR 完成，腿长方向的支撑与缓冲由 VMC 生成，侧向姿态则通过滚转稳定环进行补偿。这样处理可以把不同方向、不同时间尺度上的控制任务分开，避免在样机端引入过重的在线优化计算，同时保留足够的参数调节空间。后续 MuJoCo 仿真和第五章物理样机实验均基于该控制框架展开。

\section{总体架构}

双轮足机器人在运行时需要同时处理多个控制目标。沿前进方向，机器人应保持类似轮式倒立摆的平衡状态，并能够完成原地站立、速度跟踪和受扰恢复；在腿部方向，机构需要承担支撑、缓冲、机身高度调节以及抬腿、摆腿等动作；在侧向方向，机器人转向、经过坡面或受到横向扰动时，还需要抑制滚转角变化，避免单侧轮卸载或机体侧翻。与此同时，上述控制过程都必须在嵌入式控制器上实时运行，包括状态估计、控制量计算和执行器命令下发。

从控制能力来看，WBC、MPC 等优化型方法能够较完整地描述多任务协调和约束条件，但它们通常依赖更精确的整机模型，也需要较高的在线计算能力。对于本文样机平台而言，这类方法的实现成本偏高，调试周期也较长。相比之下，LQR 更适合处理平衡点附近的姿态稳定、速度调节和腿摆控制量分配，VMC 则便于在腿长方向构造支撑和缓冲特性。因此，本文没有采用统一优化框架，而是根据控制任务对应的运动方向和执行机构进行分层设计，在满足控制效果的同时降低在线求解压力。
...
在该架构中，LQR、VMC 和滚转稳定环分别作用于不同的控制层级。LQR 根据线性化模型计算轮端力矩和腿摆方向控制量，用于维持俯仰平衡、速度响应和转向运动；VMC 根据腿长误差生成轴向支撑力，使腿部具有类似弹簧阻尼器的支撑和缓冲特性；滚转稳定环则在左右腿长参考值中加入差动修正，用于减小侧向姿态偏差。这样划分后，每个控制环节的输入、输出和物理含义都比较明确，后续仿真分析和样机调参也更容易定位问题。
...
\subsection{与已有控制方案的对比}

为进一步说明本文分层架构的设计逻辑，表\ref{tab:method-comparison} 从控制结构、平衡策略、腿长控制、侧向稳定方式和计算特征五个维度，将本文方案与两个代表性工作进行对比。

由表\ref{tab:method-comparison} 可见，不同控制方案的主要区别在于平衡控制、腿部支撑和侧向稳定是否在同一优化框架中统一处理。Klemm 等人的 LQR-WBC 方法将 LQR 反馈律作为 WBC 中的一个运动任务，并与姿态约束、接触约束等共同求解，适合描述多任务之间的优先级关系，但在线 QP 求解会增加实时计算负担\cite{klemm2020lqr}。Yang 等人的 Adaptive LQR-VMC 方法在 LQR-VMC 结构中引入权重自适应调节，使控制器能够随状态变化调整反馈特性，但同时需要额外的在线运动学求解和参数更新\cite{yang2024integrated}。因此，上述两类方法在提高任务协调能力或控制适应性的同时，也相应增加了控制系统的实现复杂度。

\begin{table}[H]
  \centering
  \caption{本文控制方案与已有方法的对比}
  \label{tab:method-comparison}
  \small
  \begin{tabular}{p{2.2cm}p{3cm}p{3.2cm}p{3.2cm}}
    \toprule
    对比维度 & Klemm 2020\cite{klemm2020lqr} (LQR-WBC) & Yang 2024\cite{yang2024integrated} (Adaptive LQR-VMC) & \textbf{本文 (分层 LQR-VMC)} \\
    \midrule
    控制结构 & LQR 嵌入 WBC 层级，统一 QP 优化求解 & 自适应 LQR + VMC，权重在线调整 & LQR、VMC、滚转补偿三层独立，无在线优化 \\
    \hline
    平衡策略 & LQR 反馈律作为 WBC 任务，生成"先后退再前进"的加速行为 & 自适应 LQR 根据状态误差在线调整 Q 矩阵权重 & 固定增益 LQR 反馈律，离线求解 Riccati 方程 \\
    \hline
    腿长控制 & WBC 统一优化分配，与平衡、姿态任务耦合 & VMC 虚拟弹簧-阻尼力 + 重力补偿 & VMC 虚拟弹簧-阻尼力，独立于平衡回路 \\
    \hline
    侧向稳定 & 基于 ZMP 位置调节车体侧倾角 & 转向补偿器 + 重力补偿器 & 基于滚转角的几何关系修正左右腿长差 \\
    \hline
    计算复杂度 & 高（QP 在线求解，O($n^3$) 最坏情况） & 中（自适应权重计算 + FK 在线求解） & 低（矩阵乘法 O($n^2$) + 闭式代数运算） \\
    \hline
    物理验证 & Ascento 样机实验，含越障与抗扰测试 & 仿真为主，物理实验较初步 & 物理样机闭环实验，含平衡、速度跟踪与滚转稳定 \\
    \bottomrule
  \end{tabular}
\end{table}


本文面向小型样机的嵌入式实时控制需求，采用固定增益 LQR、VMC 腿长支撑和滚转角几何补偿相结合的分层结构。LQR 主要作用于矢状面内的平衡与速度跟踪，VMC 通过腿长误差构造轴向支撑力，滚转补偿则根据机体侧倾状态修正左右腿长参考，其中 LQR 和 VMC 均已被用于轮腿或足式机器人的平衡与柔顺控制研究\cite{klemm2020lqr,pratt2001}。三者分别对应不同的控制目标和执行通道，在线阶段不需要通过统一优化器进行联合求解。这样处理不能像 WBC 或 MPC 那样显式描述所有任务约束之间的耦合关系\cite{klemm2020lqr,dicarlo2018mpc}，但能够使各控制环节的输入、输出和计算量保持清晰可控。对于本文样机而言，该结构有利于降低调试难度，并保证控制周期内的状态估计、反馈计算和力矩映射能够稳定完成。
...
\begin{equation}
  ...
  \label{eq:lqr-cost}
\end{equation}
其中，$\boldsymbol{Q} \succeq 0$ 为状态加权矩阵，$\boldsymbol{R} \succ 0$ 为控制加权矩阵。$\boldsymbol{Q}$ 用于度量前进位移、偏航角、腿摆角和机体俯仰角等状态的偏差程度；$\boldsymbol{R}$ 用于约束轮电机与腿部驱动器的输出幅值，避免过大的控制输入导致执行器饱和和结构冲击。
...
\subsection{LQR 增益矩阵分析}

$\boldsymbol{Q}$ 和 $\boldsymbol{R}$ 矩阵的选取直接影响 LQR 的闭环性能。本文按照以下原则配置权重：(1) 俯仰角 $\theta_b$ 和角速度 $\dot{\theta}_b$ 的权重最高，以保证平衡恢复优先于速度跟踪；(2) 控制输入权重按驱动器的额定力矩和响应带宽设定，轮电机的控制代价低于腿部电机；(3) 前进位移 $s$ 和偏航角 $\phi$ 的权重相对较低，以避免在小速度误差时产生过大的瞬时驱动指令。

为评估增益矩阵对权重变化的敏感程度，考虑 $\boldsymbol{Q}$ 和 $\boldsymbol{R}$ 中关键元素的缩放。设标称权重矩阵为 $\boldsymbol{Q}_0$、$\boldsymbol{R}_0$，对应的 Riccati 解为 $\boldsymbol{P}_0$，反馈增益为 $\boldsymbol{K}_0 = \boldsymbol{R}_0^{-1}\boldsymbol{B}^{\mathsf{T}}\boldsymbol{P}_0$。当俯仰角误差权重 $q_\theta$ 从标称值变化至 $\alpha \cdot q_\theta$（$\alpha \in [0.5,\,2.0]$）时，闭环极点会相应移动，但系统稳定所需的条件远不限于单一参数的具体取值——只要 $\boldsymbol{Q}$ 和 $\boldsymbol{R}$ 保持正定（半正定），Riccati 方程的解连续依赖于权重矩阵的元素，反馈增益 $\boldsymbol{K}$ 也因此随权重连续变化，不会发生突变。这一连续性保证了参数在一定范围内波动时，闭环系统的稳定性不会被破坏。

从频域鲁棒性来看，LQR 具有固有裕度：对于全状态反馈的无限时域 LQR，反馈系统在输入端具有至少 6\,dB 的增益裕度和至少 $60^\circ$ 的相位裕度\cite{kalman1960new}。这意味着在建模误差或参数摄动导致实际环路增益偏移标称值 $0.5\sim2.0$ 倍的范围内，系统名义上仍可保持闭环稳定。这一特性对轮腿机器人的工程实现具有实际意义——样机装配公差、腿部等效惯量偏差和电机力矩常数误差等因素本质上都会作用于系统的等效增益，LQR 的固有裕度为这类不确定性提供了缓冲空间。

在 MuJoCo 仿真中，本文对 $\boldsymbol{Q}$ 中俯仰角误差权重和 $\boldsymbol{R}$ 中轮电机控制权重分别进行了 ±50\% 范围内的扫描测试。结果表明，在该变化范围内，平衡恢复时间和超调量的变化不超过标称值的 25\%，稳态误差几乎不受影响。但当俯仰角权重低于标称值的 30\% 时，恢复响应明显变慢，表明平衡任务的优先级下降过多会影响整体性能。这一趋势与理论预期一致：$\boldsymbol{Q}$ 与 $\boldsymbol{R}$ 的相对比例（而非绝对值）决定反馈刚度，只要比例在合理区间内，闭环行为不会剧烈变化。上述讨论既解释了本文选取标称权重的依据，也给出了参数整定的容差范围。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.45\textwidth]{figures/ch04/vmc.png}
  \caption{VMC 虚拟控制示意图}
  \label{fig:vmc-response}
\end{figure}
...
\end{equation}
其中，$\boldsymbol{\tau}_{i}$ 为第 $i$ 条腿对应主动关节的驱动力矩向量。\eqref{eq:leg-joint-torque} 将“任务空间中的腿长支撑力与 LQR 腿摆控制量”转换为“关节空间中的可执行力矩命令”，使闭环五连杆机构仍可在虚拟腿描述下进行直观控制。
...
\end{equation}
其中，$l_d$ 为对称支撑工况下的公共基准腿长。随后将 $l_{d,l}$ 和 $l_{d,r}$ 代入\eqref{eq:leg-force-law} 对应的左右腿腿长控制器，即可得到滚转稳定后的腿长控制量。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/ch04/mujoco_robot.jpg}
  \caption{MuJoCo 双轮足机器人仿真模型}
  \label{fig:mujoco-model}
\end{figure}

仿真中的控制器以 $1\,\mathrm{kHz}$ 的频率更新。每个周期开始时，程序从 MuJoCo 读取机体姿态、轮角、轮速、腿部关节角和关节角速度，并据此计算前进位移、前进速度、虚拟腿长、腿摆角及其变化率。状态量更新后，控制器依次完成 LQR 反馈、VMC 腿长支撑计算和任务空间到关节空间的力矩映射，再把轮端力矩和腿部关节力矩写入对应执行器。仿真端采用与样机控制程序相同的信号流，便于观察控制律在接触、惯性耦合和执行器限幅同时存在时的闭环响应。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/ch04/balance_response.png}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.8\textwidth]{figures/ch04/velocity_tracking.png}
...
首先，从控制目标和工程约束出发，说明了采用”LQR 运动 + VMC 腿长支撑 + 滚转稳定”这一分层架构的原因，并通过与 Klemm 2020 (LQR-WBC) 和 Yang 2024 (Adaptive LQR-VMC) 的理论对比，阐明了本文方案在控制结构、计算复杂度和工程实现路径上的差异。随后，基于第三章的建模结果构建了运动控制器所需的线性状态空间模型，设计了 LQR 反馈律，并对增益矩阵的权重选取原则和参数敏感性进行了分析。在此基础上，通过 VMC 完成腿长支撑控制，并将 LQR 输出的腿摆控制量一并映射到关节空间，同时利用滚转辅助控制实现左右腿长参考修正。最后，给出了控制系统的状态估计与在线工作流程，并通过 MuJoCo 仿真对各类工况进行了验证。

从计算成本来看，本文分层架构的在线计算主要包括：(1) LQR 部分的 $10\times 10$ 矩阵乘法，复杂度为 O(100) 次浮点运算；(2) VMC 虚拟力的闭式代数计算，仅涉及平方根和少量乘加；(3) 滚转角补偿的三角函数查表；(4) 卡尔曼滤波器的矩阵运算。四个环节均无迭代收敛过程，总计算量在典型的嵌入式 MCU（如 STM32F4 系列，主频 168\,MHz）上一个控制周期内可轻松完成。作为对比，Klemm 2020 的 LQR-WBC 方案每步需求解 QP 子问题，最坏情况复杂度为 O($n^3$)；Yang 2024 的 Adaptive LQR-VMC 方案增加了在线 Riccati 更新和 FK 求解。本文方案通过放弃自适应和在线优化，换取了对嵌入式平台的天然友好性——在样机的实际运行中，整个控制回路（含状态估计、控制量计算和力矩映射）的耗时始终控制在 1\,ms 以内，远低于 5\,ms 的控制周期。上述内容共同构成了本文后续物理样机实验所依赖的控制系统实现基础。
