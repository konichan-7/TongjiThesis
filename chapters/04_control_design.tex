...
从表\ref{tab:method-comparison} 可以看出，三者在控制哲学上的核心差异在于对“耦合”的处理方式。Klemm 的方案追求理论完备性，将 LQR 作为 WBC 框架中的一个任务，与姿态、约束等联合求解，理论上可保证各任务之间的协调性，但代价是每步均需在线求解 QP，对嵌入式计算平台不够友好。Yang 的方案则在 LQR-VMC 的基础上引入了自适应性——通过在线调整 LQR 权重矩阵使控制器对状态变化更灵敏，这在一定程度上提升了性能，却增加了在线计算环节（FK 在线求解、权重自适应调整），削弱了 LQR-VMC 架构最核心的“低计算成本”优势。

本文方案选择了另一条路径：固定增益 LQR 承担平衡与跟踪，VMC 独立处理腿长支撑，滚转角补偿以几何方式计算。三者之间不通过优化器耦合，而是靠物理意义上的“频率分离”自然解耦——平衡回路的响应带宽通常在 5--10\,Hz，腿长支撑的调节频率更低（1--3\,Hz），滚转方向的变化更缓慢。这种分层方式虽然牺牲了多任务联合优化的理论完备性，但在嵌入式实时控制中获得了明确的工程优势：每一层的计算量在离线阶段即可确定，在线执行仅有矩阵乘法和数值代数运算，不存在迭代收敛问题。
