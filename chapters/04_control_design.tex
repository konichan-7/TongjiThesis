...
在该架构中，LQR、VMC 和滚转稳定环分别作用于不同的控制层级。LQR 根据线性化模型计算轮端力矩和腿摆方向控制量，用于维持俯仰平衡、速度响应和转向运动；VMC 将任务空间力矩转换到关节空间，同时根据腿长误差生成轴向支撑力，使腿部具有类似弹簧阻尼器的支撑和缓冲特性；滚转稳定环则在左右腿长参考值中加入差动修正，用于减小侧向姿态偏差。
