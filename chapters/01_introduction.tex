...
上述串联构型样机充分展示了串联腿在运动灵活性与工作空间方面的优势，但同时也暴露出共性问题：开链结构导致腿部转动惯量偏大，高速运动时对关节力矩和控制的响应速度要求较高；串联关节的逐级误差累积也降低了整机刚度与末端定位精度。Handle 和 HotWheel 均需配备较高功率密度的驱动单元来弥补刚度不足，TRON 1 虽通过可更换足端实现多形态切换，但其腿部本质上仍是全串联链，未在负载分担和侧向刚度方面进行针对性增强。
...
并联构型通过闭链机构在结构刚度与承载力方面建立了优势，Ascento 和 Ollie 均以高度紧凑的腿部设计实现了优异的动态性能。然而，这种构型的共性局限在于：闭链结构天然限制了足端工作空间，导致腿长变化范围和越障高度受限；闭式运动链对加工精度和装配误差较为敏感，机构的动力学建模也更复杂。Ascento 的四连杆虽实现了轮心的近似直线运动，但有效腿长变化范围有限，在需要大幅高度调节的场景下适应性不足。Ollie 虽通过腹部飞轮补偿了空中姿态，但增加的飞轮电机也引入了额外的质量和控制维度。
...
综合上述三种构型的代表性平台，混合式结构在理论上能够平衡串联的大工作空间与并联的高刚度优势，但从现有样机来看，这一潜力的挖掘仍不充分。Stanley 和交龙方案侧重于将驱动并联化以降低惯量，其闭环支链主要在传动端发挥作用，尚未将并联结构的承载能力充分转化为对轮地接触力和侧向刚度的直接增强。此外，目前多数混合式平台面向高性能竞赛或开源验证场景，尺寸偏大、成本偏高，尚缺少面向小型化、低成本原型验证的紧凑设计方案。
...
\begin{figure}[H]
  \centering
  \includegraphics[width=0.4\textwidth]{figures/ch01/image14.png}
...
表\ref{tab:control-method-overview} 从建模要求、计算特征和嵌入式适用性等维度，对上述主流控制方法进行了对比。
