...
本文以混合式双轮足机器人为研究对象，围绕机械结构设计、运动学与动力学建模、分层平衡控制以及仿真与样机实验验证展开研究。机械结构方面，完成整机构型、闭环腿部机构、传动系统和驱动单元的设计与参数确定；建模分析方面，建立机器人平面等效运动学与动力学模型，推导线性化状态空间方程，并给出五连杆机构的任务空间到关节空间力矩映射；控制方法方面，提出 LQR-VMC 分层控制架构，利用 LQR 实现机体平衡、速度跟踪和腿摆控制，利用 VMC 实现腿长方向支撑，并通过滚转角差动补偿提高侧向稳定性；实验验证方面，在 MuJoCo 仿真和物理样机平台上对原地平衡、速度跟踪、扰动恢复、腿长调节和滚转稳定等工况进行验证。全文共六章，具体章节安排如下。
