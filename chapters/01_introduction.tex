...
机械结构方面：串联构型工作空间大，但腿部转动惯量高、刚度不足；并联构型承载能力强、结构紧凑，但运动范围受限、机构复杂；混合式构型虽试图兼顾两者优势，但现有平台多面向高性能样机验证，在紧凑化、低成本实现方向上的探索尚不充分。

控制方法方面：WBC、MPC 和强化学习等主流策略在复杂任务中控制效果突出，但对高精度模型、在线优化计算或大量训练数据的依赖，使它们在嵌入式平台上面临部署困难、实时性不足等问题。对于以工程实现为目标的双轮足机器人，能否在保持平衡、越障和侧向稳定性的同时，降低控制算法的复杂度与实现门槛，仍是一个尚未充分解决的问题。
...
第4章设计 LQR-VMC 分层控制器，包含 LQR 平衡-跟踪模块、VMC 腿长支撑模块和滚转稳定补偿模块，给出状态估计与控制流程，并通过 MuJoCo 仿真验证闭环性能。
