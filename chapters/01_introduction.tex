...
从表\ref{tab:control-method-overview} 可见，WBC 和 MPC 的优越性能以高昂的在线计算为代价，RL 则将复杂性转移到训练阶段。面向嵌入式实时控制的工程需求，LQR 和 VMC 的低计算成本优势突出——前者离线求解 Riccati 方程，在线仅需计算恒定增益矩阵的乘法；后者为闭式代数运算，无需迭代。LQR 和 VMC 的结合能够在保持平衡和腿长支撑两个方向上分别发挥各自优势，且不引入在线优化环节。
