...
本章给出后续控制设计所需的建模基础，重点包括两部分：一是将第二章的腿部闭环机构做等效处理，把单腿抽象为矢状面内的可伸缩虚拟杆；二是在统一坐标系与符号约定下，推导平面等效模型的运动学与动力学方程。最后，给出五连杆机构在 VMC 框架下从任务空间到关节空间的运动学映射与力矩表达式，便于第四章直接开展控制器设计与参数整定。
...
为保持全文符号体系统一，先给出原始广义坐标与输入，再引入用于平面等效建模与 VMC 推导的变量。原始广义坐标取为
\begin{equation}
  \boldsymbol{q} =
  \begin{bmatrix}
...
同理，髋关节速度与虚拟腿等效质心速度也可由对应位置向量对广义坐标求导得到。可以看到，$l$ 与 $\theta_l$ 不仅决定髋关节与机体质心的位置，还会通过雅可比矩阵影响机体平动速度与俯仰的耦合关系。这一点为后续在任务空间直接调节支撑力与摆动力矩提供了运动学依据。另一方面，$\theta_l$ 与原始变量 $\theta_{ll}$、$\theta_{lr}$ 存在显式对应关系，因此平面等效模型与原始五自由度模型在记号上是可互相转换的。
...
上一节给出了用于控制设计的平面等效模型，但实际单腿仍是五连杆闭环机构，其几何构型可直接沿用第二章的机构简图（\figref{fig:leg-linkage}）。本节不再重复绘制示意图，而是在图 2.1 所示构型基础上建立“主动关节空间 $\rightarrow$ 虚拟腿任务空间”的映射关系。为与全文记号一致，将两个主动关节角记为 $\theta_{ll}$、$\theta_{lr}$，两个被动关节角记为 $\theta_{pl}$、$\theta_{pr}$，基座两铰点为 $A$、$D$，末端汇交点为 $P$，闭环约束为
\begin{equation}
...
式 \eqref{eq:vmc-torque-map} 给出了从虚拟腿任务空间到五连杆主动关节空间的力矩映射。对本文腿部机构而言，$F_l$ 对应腿长/支撑方向的输出，可用于调节腿长与法向支撑；$T_\theta$ 对应摆动方向的输出，用于驱动腿部前后摆动并辅助姿态调节。换句话说，VMC 在“任务空间目标”和“关节空间执行量”之间提供了一条可解析计算的通道，其中的任务空间姿态变量正是本文统一使用的 $\theta_l$。
...
本章完成了控制器设计所需的建模工作。针对腿部闭环机构，采用虚功等效将其抽象为可伸缩虚拟杆，并在 VMC 框架下用轴向虚拟力 $F_l$ 与摆动虚拟转矩 $T_\theta$ 描述腿部任务空间控制量。随后在统一坐标系与符号约定下，推导了平面等效模型的运动学关系，并基于拉格朗日方法给出了动力学方程及相应的质量矩阵、重力项和输入矩阵。最后，针对实际五连杆机构推导了闭环运动学、虚拟腿雅可比矩阵以及任务空间到关节空间的力矩映射，从而把“实际机构—虚拟腿任务空间—整机平面动力学模型”贯通起来，为第四章控制器设计与参数整定提供依据。
